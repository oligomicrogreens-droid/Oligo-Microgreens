    ...
    <div className="bg-white dark:bg-gray-800 p-5 rounded-lg shadow-md">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <select
          ...
          className="flex-grow px-3 py-2 text-gray-800 bg-white dark:bg-gray-600 dark:text-white border border-gray-300 dark:border-gray-500 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
        >
        ...
        <button
          ...
          className="px-4 py-2 bg-indigo-600 text-white font-semibold rounded-md shadow-sm hover:bg-indigo-700 disabled:bg-gray-400 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800 transition-colors duration-300"
        >
          Dispatch
        </button>
      </div>
...
const DispatchDashboard: React.FC<DispatchDashboardProps> = ({ orders, onDispatch, deliveryModes }) => {
  ...
  if (dispatchableOrders.length === 0) {
    return <div className="text-center py-10 text-gray-500 dark:text-gray-400">No orders ready for dispatch.</div>;
  }
  
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Dispatch Dashboard</h2>
