...
const DeliveryManifestModal: React.FC<DeliveryManifestModalProps> = ({ orders, onClose }) => {
  ...
  const groupedByDeliveryMode = useMemo<Record<string, Order[]>>(() => {
    return orders.reduce((acc, order) => {
      const mode = order.deliveryMode || 'Unassigned';
      (acc[mode] = acc[mode] || []).push(order);
      return acc;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4 print:bg-transparent print:p-0" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col print:shadow-none print:max-h-full print:max-w-full" onClick={e => e.stopPropagation()}>
        ...
        <div id="printable-manifest" className="p-6 overflow-y-auto">
          ...
          <div className="space-y-8">
            {/* FIX: Replaced Object.entries with Object.keys to resolve a type inference issue where the result of Object.entries was treated as 'unknown'. */}
            {Object.keys(groupedByDeliveryMode).map((mode) => {
              const ordersInGroup = groupedByDeliveryMode[mode];
              return (
                      ...
                      <div key={order.id} className="manifest-order bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                        <div className="manifest-order-header flex justify-between items-start mb-3">
                          <div>
                            ...
                            <p className="text-xs font-mono text-gray-500 dark:text-gray-400 flex items-center gap-1 mt-1">
        ...
        <div className="bg-gray-50 dark:bg-gray-800/50 px-6 py-4 flex justify-end space-x-3 print:hidden">
